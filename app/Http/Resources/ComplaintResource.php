    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'next_statuses' => (function () use ($request) {
                $allTransitions = Complaint::VALID_TRANSITIONS[$this->status] ?? [];
                $user = $request->user();
                // If an engineer is viewing and status is awaiting_verification → no engineer actions
                if ($user && $user->hasRole('engineer') && $this->status === Complaint::STATUS_AWAITING_VERIFICATION) {
                    return [];
                }
            ...
            'assigned_engineer' => $this->whenLoaded('assignedEngineer', fn () => $this->assignedEngineer ? [
